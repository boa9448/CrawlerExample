...
class SteamCrawler:
    ...
    def __init__(self):
        def threadFunc(categoryName, url, resultQueue):
            driver = CategoryCrawler(url, True)
            result = driver.getCategoryItems()
            resultQueue.put((categoryName, result))
            #print(result)
            del driver

        categoryList = self.getCategoriList()
        
        self.threadList = []
        self.resultQueue = queue.Queue()
        for category, url in categoryList:
            t = Thread(target = threadFunc, args = (category, url, self.resultQueue))
            self.threadList.append(t)

    def start(self):
        [t.start() for t in self.threadList]
    ...
    def getCategoriList(self, steamURL = STEAM_URL):
        try:
            #메인 페이지 요청
            response = rq.get(steamURL)
            ...
            #카테고리 링크가 속해있는 부모 블록
            parentDivBlcok = parser.select_one(self.CATEGORY_PARENT_BLOCK_SELECTOR)
        
            #부모 블록 아래의 클래스만 검색함
            childCategoryList = parentDivBlcok.find_all("a", class_ = self.CATEGORY_CHILD_CLASS)
